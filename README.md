...
1. Search for `'wordsmiths'` (inside single quotations) to capture the text domain.
2. Search for `wordsmiths_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `wordsmiths-` to capture prefixed handles.

OR

* Search for: `'wordsmiths'` and replace with: `'megatherium'`
* Search for: `wordsmiths_` and replace with: `megatherium_`
